...
int main(int argc, char *argv[]) {
  ...
  uint8_t CMD_DEVICE_INFO[8] = {'W','e','A','c','t',':', 0x02, 0x00};
  uint8_t CMD_RESET_PAGES[8] = {'W','e','A','c','t',':', 0x00, 0x00};
  uint8_t CMD_REBOOT_MCU[8] = {'W','e','A','c','t',':', 0x01, 0x00};
...
/*
  ...
  */
  ...
  memcpy(&hid_tx_buf[1], CMD_DEVICE_INFO, sizeof(CMD_DEVICE_INFO));
  printf("> Sending <read FW ver> command...\n");
  ...
  usleep(500);
  hid_read(handle, hid_tx_buf, HID_TX_SIZE); // reuse
  printf("> %s\n", hid_tx_buf);
  ...
  while(read_bytes > 0) {
    ...
    do{
      ...
      usleep(500);
    }while(hid_rx_buf[6] != 0x03); // WeAct:\x03
